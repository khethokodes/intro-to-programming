...
function openTheStand() {
  ...
  for (var i = 0; i < days.length; i++) {
    ...
    var dailyPrice = glassPrice;
    if (days[i] === "Sunday") {
      dailyPrice = glassPrice * 0.8;
      document.getElementById(
        "result"
      ).innerHTML += `<p><strong>Sunday Special!</strong> Discounted price: $${dailyPrice.toFixed(
        2
      )}</p>`;
    }

    glassesSold = Math.floor(dailyTemp[i] / dailyPrice);
